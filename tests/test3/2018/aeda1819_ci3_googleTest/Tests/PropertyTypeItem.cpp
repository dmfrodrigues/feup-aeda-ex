...
bool PropertyTypeItem::operator<(const PropertyTypeItem &pti1) const {
	//TODO:
    if     (address    != pti1.address   ) return (address    < pti1.address   );
    else if(postalCode != pti1.postalCode) return (postalCode < pti1.postalCode);
    else                                   return (typology   < pti1.typology  );
}
